...
This starts the Vite dev server; open the printed URL (default <http://localhost:5173>). The app opens on the **Overview** — a flight-envelope map of the whole suite; click a regime (or a module card) to enter it. A grouped left sidebar switches between the Overview and Modules A/B/C and **D · Landing**, and a header ☾/☀ button toggles the light/dark theme (light by default). Hot-module reload is on, so edits under `apps/web/src` refresh live.
